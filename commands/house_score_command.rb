...
class HouseScoreCommand
  ...
  def execute(content:, account:, status_id:)
    ...
    return execute_all(sign, amount, delta, account) if house == '전체'
    total = @shop_sheet_manager.add_house_score(house, delta)
    return nil unless total
    @shop_sheet_manager.log_house_score(account, house, "#{sign}#{amount}", total, "[#{house}/#{sign}#{amount}]")
    if sign == '+'
  ...
  private
  def execute_all(sign, amount, delta, account)
    results = @shop_sheet_manager.add_house_score_all(delta)
    return nil unless results
    results.each do |name, total|
      @shop_sheet_manager.log_house_score(account, name, "#{sign}#{amount}", total, "[전체/#{sign}#{amount}]")
    end
    header = if sign == '+'
